...
export type Tformat = 'hex' | 'rgb';
/**
 * The function `randomColorGenerator` generates a random color in either RGB or HEX format.
 * @param {Tformat} format - The `format` parameter in the `randomColorGenerator` function is used to
 ...
 */
export const randomColorGenerator = (format: Tformat = COLOR_HEX): string => {
	switch (format) {
		case COLOR_RGB:
