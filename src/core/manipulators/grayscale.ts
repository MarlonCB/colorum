 ...
 * @returns {string} Grayscale hexadecimal color (e.g., "#858585")
 *
 * @example
 * grayscale("#FF5733");
 * // Returns "#858585"
...
export const grayscale = (color: HexColor): string => {
  ...
  const cleanHex = stripHexPrefix(color);
  ...
  return `${HEX_STRING_PREFIX}${numberToHex(y)}${numberToHex(y)}${numberToHex(y)}`.toUpperCase();
};
